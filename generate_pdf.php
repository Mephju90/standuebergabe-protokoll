...
// file_put_contents('debug_post_dump.txt', print_r($_POST, true));
...
$mpdf = new \Mpdf\Mpdf([
  ...
  'fontdata' => $fontData + [
    'sourcesanspro' => [
      'R' => 'source-sans-pro-v21-latin-regular.ttf',
      'B' => 'source-sans-pro-v21-latin-900.ttf',
      'L' => 'source-sans-pro-v21-latin-300.ttf',
    ],
    'regelmann' => [
      'R' => 'regelmann.ttf',
    ],
  ],
  'default_font' => 'sourcesanspro', // oder 'regelmann'
]);
...
//Jobnummer
$jobNummer = $_POST['jobNumber'] ?? 'Nicht angegeben';

//Datum 
$datum = $_POST['datum'] ?? 'Kein Datum';

//Kunde
$firma = $_POST['firma'] ?? 'Nicht angegeben';

//MESSE
$name = $_POST['messeName'] ?? 'Nicht angegeben';

//ORT
$ort = $_POST['messeOrt'] ?? 'Nicht angegeben';

//MESSESTART
$messeStart = $_POST['messeStart'] ?? 'Nicht angegeben';

//MESSEENDE
$messeEnde = $_POST['messeEnde'] ?? 'Nicht angegeben';

// FIRMA
$firma = $_POST['firma'] ?? 'Nicht angegeben';

// HALLE
$halle = $_POST['halle'] ?? 'Nicht angegeben';

// STAND
$stand = $_POST['stand'] ?? 'Nicht angegeben';

// GRÖSSE
$groesse = $_POST['groesse'] ?? 'Nicht angegeben';

// TYP
$typ = $_POST['typ'] ?? 'Nicht angegeben';



// Alle Checkstatus-Werte aus dem Formular auslesen
$statuses = [
  'Boden',
  'Wände',
  'Decke',
...
// Checkstatus-Werte aus $_POST holen, falls nicht gesetzt: Standardwert ''
$checkStatusValues = [];
foreach ($statuses as $status) {
  $key = "checkStatus" . $status;
  $checkStatusValues[$status] = $_POST[$key] ?? ''; // Falls nicht gesetzt, bleibt es leer
}

// Funktion zur Auswahl des richtigen Statusbildes
function getCheckmarkImage($status)
{
  $style = 'style="width: 25px; vertical-align: middle; margin: 0 2px;"';

  $okImage  = '<img src="./img/checkBtnOK.svg" ' . $style . '>'; // <img src="./img/checkBtnOK.svg" style="width: 25px; margin-right: 30px;">';
  $xImage   = '<img src="./img/checkBtnX.svg" ' . $style . '>'; // <img src="./img/checkBtnX.svg" style="width: 25px;">';
  $okGreen  = '<img src="./img/checkBtnOK_green.svg" ' . $style . '>'; // <img src="./img/checkBtnOK_green.svg" style="width: 25px; margin-right: 30px;">';
  $xRed     = '<img src="./img/checkBtnX_red.svg" ' . $style . '>'; // <img src="./img/checkBtnX_red.svg" style="width: 25px;">';

  if ($status === 'yes') {
    return $okGreen . $xImage;
  } elseif ($status === 'no') {
    return $okImage . $xRed;
  } else {
    return $okImage . $xImage;
  }
}


//////////////////////// 
//Textarea daten holen//
////////////////////////

if ($_SERVER["REQUEST_METHOD"] == "POST") {
  // Werte aus dem POST-Array abrufen
  $text1 = $_POST['text1'] ?? 'Nicht ausgefüllt';
  $text2 = $_POST['text2'] ?? 'Nicht ausgefüllt';
  $text3 = $_POST['text3'] ?? 'Nicht ausgefüllt';
  $text4 = $_POST['text4'] ?? 'Nicht ausgefüllt';
  $text5 = $_POST['text5'] ?? 'Nicht ausgefüllt';
  $text6 = $_POST['text6'] ?? 'Nicht ausgefüllt';
  $text7 = $_POST['text7'] ?? 'Nicht ausgefüllt';
  $text8 = $_POST['text8'] ?? 'Nicht ausgefüllt';
  $text9 = $_POST['text9'] ?? 'Nicht ausgefüllt';
  $text10 = $_POST['text10'] ?? 'Nicht ausgefüllt';
  $text11 = $_POST['text11'] ?? 'Nicht ausgefüllt';
  $text12 = $_POST['text12'] ?? 'Nicht ausgefüllt';
  $text13 = $_POST['text13'] ?? 'Nicht ausgefüllt';
  $text14 = $_POST['text14'] ?? 'Nicht ausgefüllt';
}



// Unterschriften aus dem Formular auslesen
$signatureAgent    = isset($_POST['signatureAgent']) ? $_POST['signatureAgent'] : '';
$signatureCustomer = isset($_POST['signatureCustomer']) ? $_POST['signatureCustomer'] : '';

$signAgentHtml = '';
$signCustomerHtml = '';

if (!empty($signatureAgent)) {
  $signAgentHtml .= '<img src="' . $signatureAgent . '" alt="Unterschrift Agentur" style="width:300px;"></div>';
}
if (!empty($signatureCustomer)) {
  $signCustomerHtml .= '<img src="' . $signatureCustomer . '" alt="Unterschrift Kunde" style="width:300px;"></div>';
}


//HTML-Inhalt zusammenstellen
$html = '
...
<style>
  div {
    ...
    .content tbody tr td:nth-child(3) {
      text-align: center;
    }
...
<div>
<!--------------------- HEADER --------------------->
        <table class="header" style="width: 100%; table-layout: fixed; border-collapse: collapse;">
  
            <tr style="width: 100%">
              <td style="font-size: 24px; text-align: left; border-bottom: 1px solid black;">Standübergabe-Protokoll</td>
              <td></td>
              <td></td>
              <td></td>
              <td></td>
              <td style="text-align: right; border-bottom: 1px solid black;">
                <img
                  src="./img/regelmann_logo.svg"
                  alt="Logo der Firma Regelmann"
                  style="max-width: 150px;"
                />
              </td>
            </tr>
            <tr>
              <td colspan="6" style="text-align: right; border-bottom: unset">' . htmlspecialchars($datum) . '</td>
            </tr>
...
<table style="width: 100%; border-collapse: collapse; margin-top: 20px; table-layout: fixed;">
  ...
  <tr>
    <td style="width: 10%; border-bottom: 1px solid black;">Jobnummer:</td>
    <td style="width: 17%; font-weight: bold; border-bottom: 1px solid black;">' . htmlspecialchars($jobNummer) . '</td>

    <td style="width: 10%; border-bottom: 1px solid black;">Kunde:</td>
    <td style="width: 15%; font-weight: bold; border-bottom: 1px solid black;">' . htmlspecialchars($firma) . '</td>

    <td style="width: 10%; border-bottom: 1px solid black;"></td>
    <td style="width: 5%; font-weight: bold; border-bottom: 1px solid black;"></td>

    <td style="width: 0%; border-bottom: 1px solid black; text-align: left"></td>
    <td style="width: 30%; font-weight: bold; border-bottom: 1px solid black; text-align: left"></td>
  </tr>

  <tr>
    <td style="width: 10%; border-bottom: 1px solid black; padding-top: 25px">Messe:</td>
    <td style="width: 17%; font-weight: bold; border-bottom: 1px solid black; padding-top: 25px">' . htmlspecialchars($name) . '</td>

    <td style="width: 10%; border-bottom: 1px solid black; padding-top: 25px">Ort:</td>
    <td style="width: 15%; font-weight: bold; border-bottom: 1px solid black; padding-top: 25px">' . htmlspecialchars($ort) . '</td>

    <td style="width: 10%; border-bottom: 1px solid black; padding-top: 25px">von:</td>
    <td style="width: 15%; font-weight: bold; border-bottom: 1px solid black; padding-top: 25px">' . htmlspecialchars($messeStart) . '</td>

    <td style="width: 5%; border-bottom: 1px solid black; padding-top: 25px">bis:</td>
    <td style="width: 15%; font-weight: bold; border-bottom: 1px solid black; padding-top: 25px">' . htmlspecialchars($messeEnde) . '</td>
  </tr>

  <tr>
    <td style="width: 10%; border-bottom: 1px solid black; padding-top: 15px">Halle:</td>
    <td style="width: 15%; font-weight: bold; border-bottom: 1px solid black; padding-top: 15px">' . htmlspecialchars($halle) . '</td>

    <td style="width: 10%; border-bottom: 1px solid black; padding-top: 15px">Größe:</td>
    <td style="width: 15%; font-weight: bold; border-bottom: 1px solid black; padding-top: 15px">' . htmlspecialchars($groesse) . ' m²</td>

    <td style="width: 10%; border-bottom: 1px solid black; padding-top: 15px">Stand:</td>
    <td style="width: 15%; font-weight: bold; border-bottom: 1px solid black; padding-top: 15px">' . htmlspecialchars($stand) . '</td>

    <td style="width: 5%; border-bottom: 1px solid black; padding-top: 15px">Typ:</td>
    <td style="width: 15%; font-weight: bold; border-bottom: 1px solid black; padding-top: 15px">' . htmlspecialchars($typ) . '</td>
  </tr>
</table>
        ...
        <!--------------------- STAND --------------------->
        <table class="content" style="width: 100%; margin-top: 3.5rem; border-collapse: collapse;">
          <thead>
            <tr>
              <th style="border-bottom: 1px solid black; text-align: left; width: 150px;  white-space: nowrap; overflow: hidden;">
                Stand
              </th>
              <th style="border-bottom: 1px solid black; width: 80px; text-align: center;">
                Status
              </th>
              <th style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">
                Bemerkung
              </th>
            </tr>
          </thead>
          <tbody>
            <tr style="width: 100%">
              <td style="border-bottom: 1px solid black;">Boden</td>
              <td style="border-bottom: 1px solid black; ">' . getCheckmarkImage($checkStatusValues["Boden"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text1, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;">Wände</td>
              <td style="border-bottom: 1px solid black;">' . getCheckmarkImage($checkStatusValues["Wände"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text2, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;">Decke</td>
              <td style="border-bottom: 1px solid black;">' . getCheckmarkImage($checkStatusValues["Decke"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text3, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;">Licht</td>
              <td style="border-bottom: 1px solid black;">' . getCheckmarkImage($checkStatusValues["Licht"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text4, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;">Strom</td>
              <td style="border-bottom: 1px solid black;">' . getCheckmarkImage($checkStatusValues["Strom"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text5, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
          </tbody>
        </table>
        ...
        <table class="content" style="width: 100%; margin-top: 2rem">

          <thead>
            <tr>
              <th style="border-bottom: 1px solid black; text-align: left; width: 150px;">
                Küche
              </th>
              <th style="border-bottom: 1px solid black; text-align: center; width: 80px">
                Status
              </th>
              <th style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">
                Bemerkung
          ...
          <tbody>
            <tr>
              <td style="border-bottom: 1px solid black;">Kühlschrank</td>
              <td style="border-bottom: 1px solid black; text-align: center;">' . getCheckmarkImage($checkStatusValues["Kühlschrank"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text6, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;">Geschirrspüler</td>
              <td style="border-bottom: 1px solid black; text-align: center;">' . getCheckmarkImage($checkStatusValues["Geschirr"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text7, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;">Kaffeemaschine</td>
              <td style="border-bottom: 1px solid black; text-align: center;">' . getCheckmarkImage($checkStatusValues["Kaffee"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text8, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;">Herd</td>
              <td style="border-bottom: 1px solid black; text-align: center;">' . getCheckmarkImage($checkStatusValues["Herd"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text9, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
          </tbody>
        </table>
        ...
        <table class="content" style="width: 100%; margin-top: 3rem">
          <thead>
            <tr>
              <th style="border-bottom: 1px solid black; text-align: left; width: 150px;">
                Multimedia
          ...
          <tbody>
            <tr>
              <td style="border-bottom: 1px solid black;">Kundeneigentum</td>
              <td style="border-bottom: 1px solid black; text-align: center;">' . getCheckmarkImage($checkStatusValues["Kundeneigentum"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text10, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;">Miete</td>
              <td style="border-bottom: 1px solid black; text-align: center;">' . getCheckmarkImage($checkStatusValues["Miete"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text11, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
          </tbody>
        </table>
        ...
        <table class="content" style="width: 100%; margin-top: 2rem">
          ...
          <tbody>
            <tr>
              <td style="border-bottom: 1px solid black;">Schlüssel</td>
              <td style="border-bottom: 1px solid black; text-align: center;">' . getCheckmarkImage($checkStatusValues["Schlüssel"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text12, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;">Pflanzen</td>
              <td style="border-bottom: 1px solid black; text-align: center;">' . getCheckmarkImage($checkStatusValues["Pflanzen"]) . '</td>
              <td style="border-bottom: 1px solid black; text-align: left; padding-left: 30px">' . htmlspecialchars($text13, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
          </tbody>
        </table>
        ...
        <table class="content" style="width: 100%; margin-top: 2rem">
          ...
          <tbody>
            <tr>
              <td style="border-bottom: 0px solid black; padding-top: 20px" colspan="4">' . htmlspecialchars($text14, ENT_QUOTES, "UTF-8") . '</td>
            </tr>
          </tbody>
        </table>
        ...
        <table class="unterschriften" style="width: 100%; margin-top: 1.5cm; ">
          <tr>
            <td style="text-align: left">
              <h4>Unterschrift Agentur</h4>
              ' . $signAgentHtml . '
            </td>
            <td style="border-bottom: unset; width: 50px"></td>
            <td class="pad2">
              <h4>Unterschrift Kunde</h4>
              ' . $signCustomerHtml . '
            </td>
          </tr>
        </table>
...
$mpdf->WriteHTML($html);
$pdfContent = $mpdf->Output('$filename', 'I');

// PDF speichern im Ordner "pdfs"
// $filename = 'standuebergabe_protokoll_' . preg_replace('/\s+/', '_', $firma) . '.pdf';
...
// Anstatt JSON auszugeben, wird eine HTML-Seite generiert, 
// die ein Popup anzeigt und das PDF in einem neuen Tab öffnet.
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <script>
    window.onload = function() {
      ...
      // Öffne das gespeicherte PDF in einem neuen Tab
      window.open("pdfs/<?php echo $filename; ?>", "_blank");
      // Optional: nach 3 Sekunden zur ursprünglichen Seite zurückleiten
      setTimeout(function() {
          ...
          <div class="row text-center">
            <div class="col logo">
              ...
              <svg
                ...
                y="0px"
                viewBox="0 0 245 51"
                enable-background="new 0 0 245 51"
                xml:space="preserve">
                <g>
                  <g>
                    <path
                      fill="#DC002F"
                      d="M220,3c0-1.7,1.3-3,2.9-3c1.6,0,2.9,1.4,2.9,3v20.7c0,1.7-1.3,3-2.9,3c-1.6,0-2.9-1.4-2.9-3V3z" />
                  </g>
                  <g>
                    <path
                      fill="#DC002F"
                      d="M229.6,3c0-1.7,1.3-3,2.9-3c1.6,0,2.9,1.4,2.9,3v20.7c0,1.7-1.3,3-2.9,3c-1.6,0-2.9-1.4-2.9-3V3z" />
                  </g>
                  <g>
                    <path
                      fill="#DC002F"
                      d="M239.2,3c0-1.7,1.3-3,2.9-3s2.9,1.4,2.9,3v20.7c0,1.7-1.3,3-2.9,3s-2.9-1.4-2.9-3V3z" />
                  </g>
                </g>
                <g>
                  <path
                    fill="#74797E"
                    d="M0,13.1h2l0.2,4.7h0.2c1.7-3.1,4.2-5.4,7.2-5.4c1,0,1.7,0.1,2.6,0.5l-0.5,2.1c-0.9-0.3-1.3-0.4-2.3-0.4
               c-2.3,0-5,1.8-7,6.5v17.4H0V13.1z" />
                  <path
                    fill="#74797E"
                    d="M24.5,12.5c6.1,0,9.8,4.3,9.8,11.8c0,0.6,0,1.3-0.1,1.9H15.8c0.1,6.4,3.9,11.1,9.6,11.1
               ...
               c0-6.5-2.9-9.9-7.5-9.9c-4.4,0-8.2,3.8-8.8,9.9H32.1z" />
                  <path
                    fill="#74797E"
                    d="M42.1,37.7v-0.2c-1.3-0.7-2.2-2.1-2.2-4.1c0-2.3,1.6-3.9,2.8-4.7v-0.2c-1.6-1.4-3.2-3.9-3.2-7
               ...
               C51.9,28.7,55,25.8,55,21.5z" />
                  <path
                    fill="#74797E"
                    d="M73.7,12.5c6.1,0,9.8,4.3,9.8,11.8c0,0.6,0,1.3-0.1,1.9H65c0.1,6.4,3.9,11.1,9.6,11.1
               ...
               c-4.4,0-8.2,3.8-8.8,9.9H81.3z" />
                  <path
                    fill="#74797E"
                    d="M89.6,0.4H92v35.2c0,1.1,0.5,1.6,1.1,1.6c0.2,0,0.4,0,1-0.1l0.4,1.9c-0.5,0.2-1,0.3-1.7,0.3
               c-2.1,0-3.1-1.3-3.1-4V0.4z" />
                  <path
                    fill="#74797E"
                    d="M100.4,13.1h2l0.2,3.9h0.2c2.3-2.6,5.2-4.6,8-4.6c4.1,0,6.2,2.1,7.1,5.2c2.9-3.2,5.7-5.2,8.7-5.2
               ...
               c-2.3,0-4.8,1.6-7.7,4.8v19.3h-2.3V13.1z" />
                  <path
                    fill="#74797E"
                    d="M156.2,22.6c0.1-3.9-1-8.1-6-8.1c-3.5,0-6.4,1.7-8.1,3l-1.1-1.8c1.8-1.3,5.3-3.3,9.5-3.3
               c6.1,0,8.1,4.4,8.1,9.8v16.3h-2l-0.3-3.3h-0.1c-2.6,2.1-5.7,3.9-9,3.9c-4,0-7.4-2.2-7.4-7C139.7,26.6,144.9,23.9,156.2,22.6z
                M147.5,37.3c2.9,0,5.5-1.4,8.7-4.1v-8.7c-10.5,1.2-14,3.7-14,7.7C142.2,35.8,144.6,37.3,147.5,37.3z" />
                  <path
                    fill="#74797E"
                    d="M165.6,13.1h2l0.2,3.9h0.2c2.6-2.6,5.3-4.6,8.8-4.6c5.3,0,7.7,3.2,7.7,9.8v16.4h-2.3V22.6
               c0-5.4-1.7-8-5.9-8c-3,0-5.2,1.6-8.4,4.8v19.3h-2.3V13.1z" />
                  <path
                    fill="#74797E"
                    d="M191.5,13.1h2l0.2,3.9h0.2c2.6-2.6,5.3-4.6,8.8-4.6c5.3,0,7.7,3.2,7.7,9.8v16.4H208V22.6c0-5.4-1.7-8-5.9-8
